---
title: "Data Codebook for 'Ticket Splitting in a Nationalized Era'"
...
.parquet files are large datasets and represent the main cast vote record data. You will need the `arrow` package to read these files (see the overall README on that).
...
`maricopa`, `maryland`, and `palmbeach` are cast vote records from the three other states I examine in the paper.
Only `maryland` is a parquet; the two others are csvs. These are organized in wide format, so each row represents a voter instead of a choice on the long ballot. These are analyzed in scripts starting with `10_`.
